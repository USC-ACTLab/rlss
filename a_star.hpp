      ...
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
        ...
        if (closedSet.find(neighbor.state) == closedSet.end()) {
          ...
          if (iter == stateToHeap.end()) { // Discover a new node
            ...
            // std::cout << "  this is a new node " << fScore << "," << tentative_gScore << std::endl;
          } else {
            auto handle = iter->second;
            // std::cout << "  this is an old node: " << tentative_gScore << "," << (*handle).gScore << std::endl;
          ...
          // TODO: this is not the best way to update "cameFrom", but otherwise default c'tors of State and Action are required
