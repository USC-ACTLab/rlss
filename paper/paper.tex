...
%
% Robust Trajectory Execution
% 
\title{Robust Trajectory Execution for Multi-Robot Teams Using Distributed Real-time Replanning}% in Multi-Robot Systems}

% Distributed Real-time Trajectory Replanning for Robust Trajectory Execution
% Robust Trajectory Execution for Distributed Multi-Robot Systems
% Robust Trajectory Execution for Distributed UGV Teams
% Robust Trajectory Execution for Distributed Multi-Robot Teams using Hybrid Real-time Planning
% Robust Trajectory Execution for Distributed Multi-Robot Teams using Hybrid Real-time Replanning
% Robust Trajectory Execution for Distributed Multi-Robot Teams Using Real-time Replanning
...
%We propose robust trajectory execution as an extension of cooperative collision avoidance that takes pre-planned trajectories directly into account.
%Our algorithm compensates for a variety of dynamic changes, including imperfect motion execution of robots, newly appearing obstacles, robots breaking down, or external disturbances.
...
%We also apply our dynamic receding horizon approach to ORCA and show that it helps to avoid deadlocks more effectively in that case, although the ORCA-based solution does not support higher-order dynamic limits.
...
%It also compensates for environmental changes and imperfect trajectory execution.
...
%Polynomial trajectories can be generated using a QP~\cite{richterISRR} without taking obstacles directly into account.
%QP-based formulations on B-splines or B\'ezier curves have been used to consider obstacles in form of linear hard constraints~\cite{flores,tang}.
...
\begin{align*}
    ...
    R(\vp)&:\text{ convex collision shape of any robot at position $\vp$},\\ %(we use a sphere with }
     %     & \text{radius $r_s$, but $R(\vp)$ can be any convex shape), and}\\
...
%The reason for infeasibilities is explained in Section~\ref{theoreticalGuarantees} and the reason for numerical issues is explained in Section~\ref{simulationSection}.}
...
A supplemental video containing some of our simulations and physical experiments is available at \url{https://youtu.be/LbWRvLfdwTA}.%https://youtu.be/Q96rTWdgSGQ}.
...
\begin{table}
    ...
    %$t_{\text{avg}}$ is the average time of computation in each iteration, $v_{min}$ is the minimum percentage of being in a collision-free (valid) configuration among all robots, and $s$ is the number of robots that reach to their destinations in the end.
    }
...
%A video showing some experiments is available at \url{TODO}.
